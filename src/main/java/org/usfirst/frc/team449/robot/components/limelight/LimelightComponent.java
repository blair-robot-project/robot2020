...
public class LimelightComponent implements DoubleSupplier {
   ...
   */
  @JsonCreator
  public LimelightComponent(@JsonProperty(required = true) final ReturnValue value, double offset) {
   ...
   */
  public static void setPipeline(final int index) {
    final LimelightComponent pipeline =
        new LimelightComponent(LimelightComponent.ReturnValue.pipeIndex, 0);
  ...
  public double getAsDouble() {
    ...
    final double[] camtran = entry.getDoubleArray(new double[6]);
    switch (value) {
      case poseX:
