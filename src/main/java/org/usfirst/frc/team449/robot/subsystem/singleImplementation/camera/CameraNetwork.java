...
public class CameraNetwork extends SubsystemBase implements Loggable {
  ...
  public CameraNetwork(
      @JsonProperty(required = true) final int serverPort,
      @NotNull @JsonProperty(required = true) final String serverName,
      @NotNull @JsonProperty(required = true) final List<MappedUsbCamera> cameras) {
    // Logging
  ...
  public void setCamNum(final int camNum) {
    this.camNum = camNum;
  }
}
