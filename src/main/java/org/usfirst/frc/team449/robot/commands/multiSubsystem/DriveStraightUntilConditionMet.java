...
/**
 * Drive straight using the NavX gyro stabilization until a {@link SubsystemConditional}'s condition
 * is met.
 */
@JsonIdentityInfo(generator = ObjectIdGenerators.StringIdGenerator.class)
public class DriveStraightUntilConditionMet<
    T extends Subsystem & DriveUnidirectional & SubsystemAHRS>
    extends PIDAngleCommand {

  /** The drive subsystem this command controls. */
  @NotNull private final T drive;
   ...
   */
  @JsonCreator
  public DriveStraightUntilConditionMet(
      ...
      @JsonProperty(required = true) final double driveVelocity) {
    super(
        absoluteTolerance,
        onTargetBuffer,
        minimumOutput,
  ...
  public void initialize() {
    Shuffleboard.addEventMarker(
        "DriveStraightUntilConditionMet init",
  ...
  public void end(final boolean interrupted) {
    if (interrupted) {
      Shuffleboard.addEventMarker(
          "DriveStraightUntilConditionMet interrupted!",
          this.getClass().getSimpleName(),
          EventImportance.kNormal);
    }
    drive.fullStop();
    Shuffleboard.addEventMarker(
        "DriveStraightUntilConditionMet end",
        this.getClass().getSimpleName(),
        EventImportance.kNormal);
  }
}
