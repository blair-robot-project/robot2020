...
/**
 * Turns on the limelight LEDs and starts scanning for a target
 * When one is found, it overrides the default drive command and turns to that target
 */
public class ScanThenTurn extends SequentialCommandGroup {
     ...
     * @param limelightCommand the command that uses the limelight (turn to vision, drive to vision, etc)
     *                         as of 2020, only one is turnToAngleLimelight, so that's what this should be
     * @param driverPipe the pipeline index that turns the LEDs off and which the driver uses as a camera
     */
    @JsonCreator
    public ScanThenTurn(@JsonProperty(required = true) int scannerPipe,
