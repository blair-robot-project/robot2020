...
public class LEDComponent {
    ...
    @JsonCreator
    public LEDComponent(@JsonProperty(required = true) int port,
                        @Nullable Integer LEDCount){
    ...
    public void setSpecificRangeHSV(int lowerBound, int upperBound, int h, int s, int v){
        ...
        if(upperBound < buffer.getLength()){
            Shuffleboard.addEventMarker("LED Controller", "Set range larger than set strip length! defaulting to set strip length", EventImportance.kTrivial);
