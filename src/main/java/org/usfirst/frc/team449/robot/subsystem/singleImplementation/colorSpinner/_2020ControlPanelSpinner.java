...
public class _2020ControlPanelSpinner extends SubsystemBase implements SubsystemConditional, Loggable {
  /**
   * The colors of the control panel wedges in the order that they appear when spinning the control
   * panel clockwise. This is the reverse order in which they actually appear on the control panel.
   * The control panel as viewed from above is depicted below.
   *
   * <pre>
   *   Y  B
   * R      G
  ...
  /**
   * Does nothing, as updating is handled by {@link _2020ControlPanelSpinner#periodic()}
   */
  @Override
  public void update() {}
  ...
  @Log
  private String _getTargetColor() {
    return String.valueOf(this.getTargetColor());
  }

  @Log
  private String _get() {
    final var result = new StringBuilder();
    this.colorMapping.forEach((spinnerColor, actualColor) -> {
      result.append(spinnerColor);
      result.append(": ");
      result.append(Util.valueOf(actualColor));
      result.append('\n');
    });
